...
export default function RightAddButton() {
  ...
  const {business_code, buisness_year, baseline_create_date, area_business, invoice_id, cust_number, doc_id, due_in_date, total_open_amount, posting_date, document_type, cust_payment_terms,invoice_currency,clear_date, document_create_date, posting_id} = user;
  ...
  const submitHandler = async (e) =>{
      ...
      if (response) {
          setUser({business_code:'',buisness_year:'', baseline_create_date:'', area_business:'', invoice_id:'', cust_number:'', doc_id:'', due_in_date:'', total_open_amount:'', posting_date:'', document_type:'', cust_payment_terms:'', invoice_currency:'',clear_date:'', document_create_date:'', posting_id:''});
        }
...
<Button style = {{width : 120, height : 50,borderColor: "white",color : "white"}} variant="outlined" onClick={handleClickOpen}>Add</Button>
      <Dialog open={open} onClose={handleClose}>
        ...
        <DialogContent>
        <AddUser
        business_code = {business_code} buisness_year = {buisness_year}
